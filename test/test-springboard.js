...
contract("Springboard", accounts => {
  ...
  it("Upgrade wallet v1 to v2 should work", async () => {
    ...
    await walletV1.die(); //selfdestruct v1 code
    ...
    const wallet2Address = tx2.logs[0].args[0];
    assert.equal(walletAddress, wallet2Address, "upgraded address mismatch");
    
    const walletV2 = await WalletV2.at(wallet2Address);
